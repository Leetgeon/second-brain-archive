...
4. 데스크톱 앱 OAuth 클라이언트에서 공개 가능한 `client_id`와 `project_id`만
   `src/second_brain_archive/youtube_oauth_public.json`에 기록합니다.

전체 다운로드 JSON과 `client_secret`은 GitHub나 공개 설치본에 포함하지
않습니다. Google은 설치 앱이 비밀을 보관할 수 없다고 전제하며 데스크톱 토큰
교환에서 `client_secret`을 선택 사항으로 정의합니다. 앱은 PKCE와 로컬 루프백
콜백을 사용합니다.
...
개인 개발 환경에서는 다음 환경변수를 사용할 수도 있습니다.

```bash
export SECOND_BRAIN_YOUTUBE_CLIENT_ID="...apps.googleusercontent.com"
export SECOND_BRAIN_GOOGLE_CLOUD_PROJECT="..."
```
